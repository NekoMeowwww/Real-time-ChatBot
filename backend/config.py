...
ws_connect_config = {
    ...
    "headers": {
        "X-Api-App-ID": "4584684169",
        "X-Api-Access-Key": "bcnGD3BJUh9g3_p50jUglX82LN4JHsSV",
...
start_session_req = {
    ...
    "dialog": {
        "bot_name": "湖州隆辕智控AI小助手",
        "system_role": "你是隶属于湖州隆辕智控有限科技公司的小助手，需要基于外部知识库来回答用户。如果你不知道答案，请礼貌地告诉用户你无法回答，而不是编造信息，如果用户提及公司有哪些设备，将知识库中有的设备及其类型告诉用户。例子：\n用户：公司有哪些设备？\n小助手：我们公司有以下设备：TR04车载北斗定位终端（不要按例子一模一样回复，只需要模仿格式）。不要编造知识库中不存在的设备！公司名为隆辕智控而不是隆源智控，不要混淆。",
...
# 字幕生成服务配置
# 注意：access_token 就是 x-api-key，用于认证
# appid 字段保留用于兼容，但实际 API 调用中不使用
caption_service_config = {
    "access_token": "d07f6e69-9a19-4891-9a45-279c9a1f3619",
    "base_url": "https://openspeech.bytedance.com/api/v1/vc"
}

# # 流式字幕服务配置
# upload_dir = "uploads"  # 音频文件上传目录
# base_url = "http://117ma447390rf.vicp.fun:38374"  # 服务器基础 URL（用于生成文件访问链接）

# 流式字幕参数
# streaming_caption_config = {
#     "chunk_duration": 10.0,  # 每多少秒生成一次字幕（秒）
#     "enabled": True  # 是否启用流式字幕
# }
